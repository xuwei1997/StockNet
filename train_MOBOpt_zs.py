...
from train_zs import trainStockNetZS
from Net import *
    ...
    def train_main(self):
        def train_main_fun(x):
            try:
                ...
                # nan
                if np.isnan(self.best_nacc) or np.isnan(self.best_mse) or np.isnan(self.best_nprecision) or np.isnan(
                        self.best_nrecall):
                    print('Nan')
                    self.best_nacc = 0.3
                    self.best_mse = 1
                    self.best_nprecision = 0.2
                    self.best_nrecall = 0.2

                # 多目标优化，最大值
                print([self.best_nacc, -self.best_mse * 10, self.best_nprecision, self.best_nrecall])
                return [self.best_nacc, -self.best_mse * 10, self.best_nprecision, self.best_nrecall]
...
def opt_main(ticker, net_model, n_trials, epoch):
    ...
    with open("log_opt_zs_" + str(ticker) + ".txt", "a", encoding="utf-8") as f:
    ...
    Optimize = mo.MOBayesianOpt(target=train_main_fun, NObj=4, pbounds=pbounds, constraints=[],
